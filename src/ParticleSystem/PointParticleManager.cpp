...
PointParticleManager::PointParticleManager(int number): m_num_of_particles(number) {
    if (number > 0){
...
*/
void PointParticleManager::changeNumParticles(int new_number) {
    if(!m_init){
...
void PointParticleManager::configureShaderProgram(){
    m_shader = ShaderLoader::createShaderProgram(":/resources/shaders/pointparticle.vert", ":/resources/shaders/pointparticle.frag");
}
...
*/
void PointParticleManager::configureVAO(){
...
/* Bind VAO, update VBO
*/
void PointParticleManager::bindAndUpdateBuffers(){
    // Bind the VAO
    glBindVertexArray(m_VAO);
...
void PointParticleManager::initializeCL(){
    setupCL();
    setupKernel();
}


void PointParticleManager::create(int id){
    m_particle_randVelOffset[id].x = 0.2*generateGaussianNoise();
    m_particle_randVelOffset[id].y = 0.2*generateGaussianNoise();
    m_particle_randVelOffset[id].z = 0.2*generateGaussianNoise();
    m_particle_randVelOffset[id].w = 0;

    m_particle_life[id] = m_max_life;
    m_particle_position[id] = m_emit_pos;
    int total_particles = m_num_of_particles/2;
    float vel_x = 3.0f*glm::cos(2*glm::pi<float>()*(float)id/total_particles);
    float vel_y = 1.0f;
    float vel_z = 3.0f*glm::sin(2*glm::pi<float>()*(float)id/total_particles);

    m_particle_velocity[id] = glm::vec4(vel_x, vel_y, vel_z, 0.0f)+m_particle_randVelOffset[id];
}

void PointParticleManager::createCircle(int id){
    m_particle_randVelOffset[id].x = 0.1*generateGaussianNoise();
    m_particle_randVelOffset[id].y = 0.1*generateGaussianNoise();
    m_particle_randVelOffset[id].z = 0.1*generateGaussianNoise();
    m_particle_randVelOffset[id].w = 0;

    m_particle_life[id] = m_max_life;
    m_particle_position[id] = m_emit_pos + glm::vec4(0.8f, 0.3f, 0.5f, 0.0f);
    int total_particles = m_num_of_particles/2;
    float vel_x = -1.0f*glm::cos(2*glm::pi<float>()*(float)id/total_particles);
    float vel_z = -1.0f*glm::sin(2*glm::pi<float>()*(float)id/total_particles);
    float vel_y = -0.3f - 1.5f*glm::cos(2*glm::pi<float>()*(float)id/total_particles);

    m_particle_velocity[id] = glm::vec4(vel_x, vel_y, vel_z, 0.0f)+m_particle_randVelOffset[id];
}


void PointParticleManager::render(const glm::mat4 &ViewProjection){
...
void PointParticleManager::update(float dt) {
...
void PointParticleManager::setupCL(){
    cl_int error;
...
void PointParticleManager::setupKernel(){
    ...
    std::filesystem::path relativePath = "src/ParticleSystem/pointparticle.cl";
...
void PointParticleManager::setupBuffer(){
...
PointParticleManager::~PointParticleManager() {
